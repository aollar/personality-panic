...
Reads Personality_Panic_Balance_Lock_v6-1.xlsx (single source of truth for numbers)
...
XLSX = r"C:\Users\aloss\OneDrive\Desktop\Personality Panic\Personality_Panic_Balance_Lock_v6-1.xlsx"
...
def parse_weekend(wb):
    ...
    for r in wb["Settings"].iter_rows(values_only=True):
        ...
        raw = r[1]
        wanted = label.startswith("Hunger") or label.startswith("Stress")
        if isinstance(raw, str) and wanted:   # a text cell must never silently zero a penalty
            m = re.search(r"([0-9.]+)\s*%", raw)
            assert m, "Settings %r holds text the parser cannot read: %r" % (label, raw)
...
def main():
    ...
    with open(OUT, "w", encoding="utf-8") as f:
        f.write("// GENERATED by scripts/build_data.py from Personality_Panic_Balance_Lock_v6-1.xlsx\n")
